...
function createMockChannel() {
  ...
  const channel = {
    on: vi.fn((type: string, filter: { event: string }, callback: (payload: { new: unknown }) => void) => {
    ...
    _simulateEvent(event: string, payload: { new: unknown }) {
      const key = `postgres_changes:${event}`;
...
describe('useRealtimeSubscription', () => {
  ...
  it('starts with disconnected status', () => {
    ...
    const { result } = renderHook(() =>
      useRealtimeSubscription({ supabase: mockSupabase, onCardUpdate })
    );
  ...
  it('sets status to connected on SUBSCRIBED', () => {
    ...
    const { result } = renderHook(() =>
      useRealtimeSubscription({ supabase: mockSupabase, onCardUpdate, onStatusChange })
    );
  ...
  it('accepts an onReconnected callback option', () => {
    ...
    const { result } = renderHook(() =>
      useRealtimeSubscription({ supabase: mockSupabase, onCardUpdate, onReconnected })
    );
